...
get_ipython().magic('run D:/Code/green-mango/Python/mangoLib/baseImports.ipynb')


# In[2]:


get_ipython().run_cell_magic('javascript', '', "$('#header').toggle()")
...
sys.path.insert(0, SRC_PATH) # add cwd to path for local module import

LOGS_DIR = os.path.join(SRC_PATH,'tf_graphs','ch14-rnn') # set Tensorflow output directory

def logs_dir_now(basePath):
    return os.path.join(basePath,datetime.now().strftime("%Y%m%d-%H%M%S"))

print('tensorboard --logdir="%s"' % logs_dir_now(LOGS_DIR))
...
with tf.Session() as sess:
    writer = tf.summary.FileWriter(logs_dir_now(LOGS_DIR), sess.graph)
